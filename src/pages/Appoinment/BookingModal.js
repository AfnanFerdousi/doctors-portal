    ...
    const handleBooking = event => {
        ...
        .then(data => {
            ...
            if(data.success){
                ...
                toast.error(`You already have an appoinmetn on ${data.booking?.date} at ${data.booking?.slot}`)
            }
            refetch();
            setTreatment(null);            
        })
            ...
            <div className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                    ...
                    <form onSubmit={handleBooking} className='grid grid-cols-1 gap-4 justify-items-center mt-5'>
                        ...
                        <input type="text"
                            name="name" disabled value={user?.displayName || ''} className="input input-bordered w-full max-w-xs" />
                        <input type="email"
                            name="email"
                        ...
                        <input type="submit" value='Submit' className="btn btn-primary font-bold text-white bg-gradient-to-r from-secondary to-primary" />
                    </form>
                </div>
            </div>
        </div>
